        ...
        (async () => {
            ...
            if (data && data.status == 200 && data.success == true) {
                if (data.response.categories) {
                    setProgress(100);
                    console.log(data);
                    const updateCategory = data.response.categories.map(x => {
                        return {
                            id: x.id ? x.id : "", image: <img src={x.image ? x.image : ""} style={{ width: 58, height: 32, backgroundColor: '#6fbfae' }} alt="Avatar" class="avatar" />, name: x.name ? x.name : "", email: "hbanks0@networkadvertising.org", username: 'user1', phone: "386-(842)278-0044", date: moment().subtract(1, 'days').format('Do MMMM YYYY'), edit: <div className="sdjsafei"><button className="btn btn-primary" onClick={() => {
