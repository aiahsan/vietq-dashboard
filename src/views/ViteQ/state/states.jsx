...
export default () => {
        ...
        (async () => {
            ...
            if (data && data.status == 200 && data.success == true) {
                if (data.response.states) {
                    setProgress(100);
                    console.log(data);
                    const updateCategory = data.response.states.map(x => {
                        return {
                            id: x.id ? x.id : "",  name: x.name ? x.name : "", edit: <div className="sdjsafei"><button className="btn btn-primary" onClick={() => {
                                history.push(BASEDIR + "/state/edit",{data:x})
    ...
    const handleDelete=async ()=>{
        setshow(false);
        setProgress(50);
        const { data, status } = await repository.delete_states({"state_id":deleteId}).then(x => x).then(x => x)
        if (data && data.status == 200 && data.success == true) {
            if (data.response) {
                setProgress(100);
                console.log(data);
                        ...
                        <div className="col-12">
                            <section className="box ">
                                <header className="panel_header">
                                    <h2 className="title float-left">States Table</h2>
