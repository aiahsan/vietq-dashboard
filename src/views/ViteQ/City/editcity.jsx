...
export default (props) => {
    ...
    useEffect(() => {
        ...
        const getData = async () => {
            const { data, status } = await repository.get_states().then(x => x).then(x => x);
            if (data && data.status == 200 && data.success == true) {
                setshowAnimation(false)
                if (data.response.states) {
                    setcategories(data.response.states);
    ...
    const postData = async (datapost) => {
        ...
        const { data, status } = await repository.edit_cities(postData).then(x => x).then(x => x)
    ...
    return (loaderMain ? <></> : <Formik
        initialValues={{
            ...
            city_id:props.location&&props.location.state&&props.location.state.data&&props.location.state.data.id?props.location.state.data.id:"",
            ...
            state_id: props.location&&props.location.state&&props.location.state.data&&props.location.state.data.state&&props.location.state.data.state.id?props.location.state.data.state.id:"",
        }}
                                                            ...
                                                            <form>
                                                                ...
                                                                <div className="form-row">
                                                                    <div className="form-group col-md-6">
                                                                        ...
                                                                        <select value={initialValues.state_id} {...getFieldProps("state_id")} onChange={(e) => {
                                                                        ...
                                                                        {touched.state_id && errors.state_id && <div style={{ color: 'red', marginTop: 10 }}>{errors.state_id}</div>}
