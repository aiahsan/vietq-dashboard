...
const header = [
    ...
    { title: 'City', prop: 'name', sortable: true, filterable: true },
...
export default () => {
    ...
    React.useEffect(() => {
        setProgress(50);
        (async () => {
            const { data, status } = await repository.get_cities().then(x => x).then(x => x)
            if (data && data.status == 200 && data.success == true) {
                if (data.response.cities) {
                    setProgress(100);
                    console.log(data);
                    const updateCategory = data.response.cities.map(x => {
                        return {
                            id: x.id ? x.id : "", name: x.name ? x.name : "", state:x.state&&x.state.name?x.state.name:"", date: moment().subtract(1, 'days').format('Do MMMM YYYY'), edit: <div className="sdjsafei"><button className="btn btn-primary" onClick={() => {
                                history.push(BASEDIR + "/city/edit",{data:x})
    ...
    const handleDelete=async ()=>{
        setshow(false);
        setProgress(50);
        const { data, status } = await repository.delete_cities({"city_id":deleteId}).then(x => x).then(x => x)
        if (data && data.status == 200 && data.success == true) {
            if (data.response) {
                setProgress(100);
                console.log(data);
